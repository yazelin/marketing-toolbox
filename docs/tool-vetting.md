...
### 2026-07-16:Wheel of Names → 抽獎轉盤(lucky-wheel)

同名單(8 人)兩邊各完成一次抽獎,皆能開出中獎者(平手);差異:

| 面向 | Wheel of Names | lucky-wheel |
|------|------|------|
| FB 留言抽獎 | 名單要自己整理 | 留言整坨貼上一鍵洗名單(贏) |
| 廣告 | 有橫幅 | 無(贏) |
| 中獎名單 | 藏側欄 Results | 專屬面板+一鍵複製(贏) |
| 公平性 | 未說明 | crypto 亂數+指針落點 12/12 對齊驗證(贏) |
| 名單隱私 | 可存雲端 | 只存瀏覽器(贏) |
| 轉盤自訂樣式/雲端保存 | 有 | 無(輸,v1 不做) |

結論:替換。驗證:`lucky-wheel/verify/check.cjs` 對線上站 exit 0(FB 清洗樣本正確、winner 屬名單、自動移除正確、無橫捲)。

### 2026-07-16:繁化姬 → 台灣化轉換器(taiwanize)

同輸入對比(「这个软件和视频都很好用,网络也稳定」):

| 面向 | 繁化姬 | taiwanize |
|------|------|------|
| 轉換結果 | 「這個軟體和影片都很好用,網路也穩定」 | 逐字相同(平手) |
| 文字隱私 | 全文送伺服器 | 純前端不上傳(贏) |
| 廣告 | 有贊助廣告 | 無(贏) |
| 操作 | 按鈕觸發 | 打字即轉(贏) |
| 詞庫深度/進階模組 | 社群詞庫+字幕/注音模組 | OpenCC 標準詞庫(輸,頁面誠實標註並保留繁化姬連結) |

結論:替換。驗證:`taiwanize/verify/check.cjs` 對線上站 exit 0(twp/tw 模式差異正確、自訂取代生效、無橫捲)。

### 2026-07-16:Metatags + OpenGraph.xyz → 分享卡預覽器(share-preview)

一個自製工具替換兩個外部工具。同網址(行銷工具箱)三邊各看預覽:

| 面向 | Metatags | OpenGraph.xyz | share-preview |
|------|------|------|------|
| FB/X/Google 預覽 | 有 | 有 | 有(平手) |
| LINE 預覽 | 無 | 無 | 有(贏,台灣行銷人剛需) |
| 編輯即預覽 | 有(含上傳圖) | 無 | 有,改網址不能傳圖(對 Metatags 小輸,誠實記) |
| meta 標籤產生 | 有 | 無 | 有,含實測 og:image 寬高(贏) |
| 介面/門檻 | 英文 | 英文+cookie 牆+付費上桿 | 繁中、無牆(贏) |
| 抓頁基礎 | 自家後端 | 自家後端 | 開源 checker Worker `raw=1`(SSRF 護欄+限流,已部署) |

結論:替換兩筆。驗證:`share-preview/verify/check.cjs` 對線上站 exit 0(四卡渲染、og:image 1200 寬、編輯即時更新、無橫捲)。

### 2026-07-17:瀏覽器去背(bye-bg)上架——與 BiRefNet demo 並存,非替換

替換鐵律誠實適用:精度上 BiRefNet(頂級模型、GPU)> bye-bg(4.6MB U2Net-p、瀏覽器 WASM),**打不贏就不替換**。
但兩者定位不同,並存各取所需:

| 面向 | BiRefNet HF demo | bye-bg |
|------|------|------|
| 精度(髮絲/半透明) | 最高 | 中,主體明確的圖夠用 |
| 圖片隱私 | 上傳 HF Space | 不離開裝置(贏) |
| 等待 | GPU 排隊 60 秒起 | 首次載 4.6MB 模型,之後秒級(贏) |
| 白底輸出(蝦皮/momo 規格) | 無 | 一鍵切換(贏) |
| 授權紅線 | — | 已避開 imgly(AGPL)與 RMBG-1.4(非商用);採 U2Net Apache-2.0 + ONNX Runtime MIT |

驗證:`bye-bg/verify/check.cjs` 對線上站 exit 0(實照 fixture 去背:透明 73.5%/主體不透明 21.2%、白底模式無透明像素、無橫捲);jsDelivr 模型 CDN 200。
